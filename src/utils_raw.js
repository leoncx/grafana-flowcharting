...
module.exports = {
  ...
  $loadFile (fname, options) {
    ...
    let authString
    if (options.remoteAuthType && options.remoteAuthType !== 'none') {
      if (options.remoteAuthType === 'basic') {
        authString = `Basic ${btoa(`${options.remoteAuthBasicUsername}:${options.remoteAuthBasicPassword}`)}`
      } else {
        authString = `Bearer ${options.remoteAuthBearerToken}`
      }
  ...
  $evalFile (fname, options) {
    const ajaxOptions = {
      method: 'GET',
      url: fname,
      async: false,
      success: data => { eval.call(window, data) },
      error: error => {
